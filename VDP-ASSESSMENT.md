...
The AC6502 as PicoCalc firmware: one UF2 per board that runs the real BIOS ROM byte for byte,
with the RTC's NVRAM kept in flash. **It is a legacy machine**, with a TMS9918-style video path
drawn to the LCD, so it takes BIOS 1.6 and stops there. Nothing in Part 2 applies to it.

## Where it stands

- Tags `v1.0.0` and `v1.0.1`.
- `src/rom/bios_rom.c` embeds `BIOS.bin` verbatim. Its header records the build
  (`v1.5-1-gd1fcefe`) and names the two things to re-check when the image changes:
  - the character set at `$B800`, which the launcher borrows (`src/launcher/launcher.c`)
  - BASIC's workspace pointers, which the program loader writes (`BAS_*` in
    `src/machine/machine.c`)
- The ROM is also replaceable from the SD card's `/ROMs` folder.
- `src/machine/rtc.c` already models NVRAM burst mode (`CTRLB_BME`, auto-increment on
  `REG_RAM_DATA`). `src/machine/nvram.c` persists the 256 bytes to flash. BIOS 1.6's save
  slots should work unchanged.

## Work outline

**Part 1, after 6502-BIOS tags `v1.6`:**

1. **Re-embed the ROM.** Dump the `v1.6` `BIOS.bin` into `src/rom/bios_rom.c` in the existing
   format, and update the header's build line.
2. **Re-check the two named dependencies.** 1.6 adds only Kernal code and `NV_ID` at `$0390`,
   so both should be unaffected. Confirm:
   - the character set at `$B800` is byte-identical
   - the `BAS_*` addresses in `BIOS.inc` are unchanged
3. **Verify on each board.**
   - Boot shows `-- 6502 BIOS v1.6 --`; BASIC and the launcher work.
   - A slot written with the save-slot routines (or the BASIC example from 6502-BIOS's README)
     survives a power cycle through the flash-backed NVRAM.
   - The clock card is still found when BME is left set, the case 1.6's `ProbeRTC` fix is for.
4. **Release** `v1.0.2` (or the next version the repo's scheme gives) with the UF2 for each
   board, and a README note that it carries BIOS 1.6.
5. **Tell 6502-DOCS** the release, so `docs/using/picocalc.md` can name it before `v1` is cut.

**Part 2:** none. The PicoCalc stays on BIOS 1.6.

## Linked repositories

| Repository | Path | Why |
|---|---|---|
| 6502-BIOS | `~/Developer/Assembly/6502-BIOS` | The `v1.6` `BIOS.bin` to embed; `PLAN.md` for the save-slot format and the `ProbeRTC` fix |
| 6502-DOCS | `~/Developer/NodeJS/6502-DOCS` | `docs/using/picocalc.md` and the `v1` banner name the PicoCalc among 1.6 machines; DOCS waits for this release before cutting `v1` |
